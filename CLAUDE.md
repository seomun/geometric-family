...
읽는 순서: 이 파일 → `HANDOFF.md` → `docs/04_IP_BUSINESS.md`(사업 흐름) → `docs/03_STYLE_GUIDE.md`(그림 규격) → 작업 대상.
...
### 1-1. 캐릭터 정본 = 시트, 배치 = 코드
세모 아내부터 **ChatGPT 시트(`assets/<char>/_sheet/`)가 정본**(D4). 생성 방법론은 `docs/05_CHATGPT_ART_PIPELINE.md` 를 글자 그대로 따른다.
