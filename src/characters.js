...
(function (root) {
  ...
  const MODE = { mode: 'clean', seed: 0, assets: '../assets/', rasterWife: true, rasterHusband: true, rasterNemoDad: true, rasterDongDad: true, rasterNemoMom: true, rasterNemoKids: true, rasterNemoGrandma: true };
  ...
  const SHEET_HAVE = { wife: ['good','joy','wink','love','surprise','worry','angry','bad','smug','tired','cry','calm'], husband: ['good','joy','wink','love','surprise','worry'], nemo_dad: ['good','joy','wink','love','surprise','worry'], dong_dad: ['good','joy','calm','warm','surprise','trouble'], nemo_mom: ['good','joy','wink','love','surprise','worry'], baby: ['good','joy','surprise','love','cry','tired'], nemo_grandma: ['good'] };
  ...
  function nemoMom(o) {
    const { x, y, w = 96, h = 92 } = o;
    if ((o.raster === undefined ? MODE.rasterNemoMom : o.raster) && !o.suit) { // ChatGPT 정본 래스터
      const cw = w * 1.52, sx = x + w / 2 - cw / 2, sy = y - h * 0.08;
      let g = sprite('nemo_mom/' + sheetEmo('nemo_mom', o.emo || 'good'), { x: sx, y: sy, w: cw, flip: o.flip || (o.gaze || 0) < 0 });
      if (o.item) g += itemAt(o.item, x + w * (o.flip ? 0.02 : 0.98), y + h * 0.72, w * 0.82);
      if (o.itemL) g += itemAt(o.itemL, x + w * (o.flip ? 0.98 : 0.02), y + h * 0.72, w * 0.82);
      return `<g class="gf-ch gf-raster">${g}</g>`;
    }
  ...
  function nemoGrandma(o) {
    const { x, y, w = 84, h = 80 } = o;
    if ((o.raster === undefined ? MODE.rasterNemoGrandma : o.raster) && !o.suit) { // ChatGPT 정본 래스터 (표정 1종)
      const cw = w * 1.5, sx = x + w / 2 - cw / 2, sy = y - h * 0.12;
      let g = sprite('nemo_grandma/good', { x: sx, y: sy, w: cw, flip: o.flip || (o.gaze || 0) < 0 });
      if (o.item) g += itemAt(o.item, x + w * (o.flip ? 0 : 1), y + h * 0.72, w * 0.85);
      return `<g class="gf-ch gf-raster">${g}</g>`;
    }
    const cx = x + w / 2, s = w / 100;
    ...
    const eye = (x, side) => {
      ...
      if (faceStyle === 'chic') {
        ...
        const iris = `<ellipse cx="${x + px}" cy="${cy + ey + 1.2 * s}" rx="${6.4 * s}" ry="${7.2 * s}" fill="#7a4a30"/><ellipse cx="${x + px}" cy="${cy + ey + 1.8 * s}" rx="${4 * s}" ry="${5 * s}" fill="${INK}"/>` +
          `<circle cx="${x + px + 2.6 * s}" cy="${cy + ey - 0.2 * s}" r="${2.7 * s}" fill="#fff"/><circle cx="${x + px - 2.4 * s}" cy="${cy + ey + 4.2 * s}" r="${1.3 * s}" fill="#fff"/>`;
        const lidY = cy + ey - ry + 2 * ry * lid; // 눈꺼풀 아래선
        const cover = `<path d="M${x - rx},${lidY} A${rx},${ry} 0 0 1 ${x + rx},${lidY}z" fill="${COLORS.semoWife}"/>`; // 몸색으로 위쪽 덮기
        const line = `<path d="M${x - rx * 0.95},${lidY + 0.5 * s} Q${x},${lidY - 3 * s} ${x + rx * 0.95},${lidY} q${side * 3.5 * s},${-1 * s} ${side * 5.5 * s},${-3.5 * s}" stroke="${INK}" stroke-width="${lw * 1.35}" fill="none" stroke-linecap="round"/>`; // 아이라인 + 꼬리
        const lash = `<path d="M${x + side * rx * 0.7},${lidY - 0.5 * s} q${side * 3 * s},${-3 * s} ${side * 4 * s},${-6.5 * s} M${x + side * rx * 0.35},${lidY - 1.5 * s} q${side * 2 * s},${-3 * s} ${side * 2.5 * s},${-6 * s}" stroke="${INK}" stroke-width="${lw * 0.7}" fill="none" stroke-linecap="round"/>`;
        const lower = `<path d="M${x + side * 3 * s},${cy + ey + ry + 1.5 * s} q${side * 4 * s},${0.5 * s} ${side * 7 * s},${-2 * s}" stroke="${INK}" stroke-width="${lw * 0.45}" fill="none" stroke-linecap="round" opacity=".8"/>`;
        return `<g transform="rotate(${tilt} ${x} ${cy + ey})"><ellipse cx="${x}" cy="${cy + ey}" rx="${rx}" ry="${ry}" fill="#fff" stroke="${INK}" stroke-width="${lw * 0.55}"/>${iris}${cover}${line}${lash}${lower}</g>`;
      }
